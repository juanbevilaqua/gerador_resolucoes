...
class TelaPrincipal:
    ...
    def check_tipo_res(self, tipo):
        ...
        if tipo == "Aproveitamento Suf.":
            self.estrurar_frame_dinamico(tipo, 1, True)
    ...
    def estrurar_frame_dinamico(self, tipo, line, primeira_chamada):
        ...
        if tipo == "Aproveitamento Suf.":
            if primeira_chamada:
                ...
                self.frame_superior.grid(row=0, column=0, padx=(0, 10), pady=10)
            ...
            #self.frame_entry_button_container.pack(side="left", padx=10, pady=10)#, fill="y"
            self.frame_entry_button_container.grid(row=line, column=0, padx=(0, 10), pady=10)
            ...
            self.frame_direito.grid(row=line, column=1, padx=(0, 10), pady=10)

            self.criar_campo("Nome do Discente", tipo='entry_button', frame=self.frame_entry_button_container)
            self.criar_campo("Data do Exame", linha=1, frame=self.frame_direito, flag_list=True)
            self.criar_campo("Resolução de Aprovação", linha=2, frame=self.frame_direito, flag_list=True)

    def criar_campo(self, nome, tipo='entry', opcoes=None, linha=0, frame=None, flag_list=False):
        if frame is None:
        ...
        if tipo == 'entry':
            ...
            entry = ctk.CTkEntry(frame_entry_button, width=200)
            entry.pack(side="left", padx=(0, 10))
            self.campos_dinamicos[f"Nome do Discente {self.cont_entry_button}"] = entry
            ...
            self.button_add = ctk.CTkButton(frame_entry_button, text="+", width=50, command=lambda: (self.estrurar_frame_dinamico("Aproveitamento Suf.", self.cont_entry_button+1, False)))
    ...
    def capturar_dados(self):
        ...
        valores_dinamicos = {}
        dados = defaultdict(list)#dicionario de listas aux
        for nome, widget in self.campos_dinamicos.items():
            if isinstance(widget, ctk.CTkEntry):
                valor = widget.get()
                partes_nome = nome.split()
                chave_dicionario = " ".join(partes_nome[:-1])#será o nome da variavel sem o contador
                ...
                if partes_nome[-1].isdigit():#verifica se há um cont acoplado ao nome do identificador
                    ...
                    dados[chave_dicionario].append(valor)#armazena os valores em um dicionario de listas auxiliar
                    valores_dinamicos[chave_dicionario] = dados[chave_dicionario]#atualiza os valores dinamicos com os valores do dicionario aux
